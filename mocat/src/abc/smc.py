...
def run_abc_smc_sampler(abc_scenario: ABCScenario,
    ...
    initial_extra = CDict(random_key=random.split(random_key, n_samps), iter=0)

    initial_state, initial_extra = vmap(
        lambda state, extra: startup_mcmc(abc_scenario, mcmc_sampler, None, mcmc_correction,
                                          state, extra), (0, CDict(random_key=0, iter=None)))(initial_state,
                                                                                              initial_extra)
    ...
    if initial_state.distance.ndim == 1:
        concat_distance = lambda dists: dists
    elif initial_state.distance.ndim == 2:
        concat_distance = lambda dists: np.max(dists, axis=1)
    else:
        raise ValueError('initial_state.distance not a 1 or 2 dimensional array')
    ...
    if preschedule is not None:
        ...
        out_samps = _run_adaptive_abc_smc_sampler(abc_scenario, mcmc_sampler, mcmc_correction, mcmc_steps,
                                                  ...
                                                  concat_distance, threshold_quantile_retain, continuation_func)
...
def _run_prescheduled_abc_smc_sampler(abc_scenario: ABCScenario,
                                      ...
                                      concat_distance: Callable) -> CDict:
...
def _run_adaptive_abc_smc_sampler(abc_scenario: ABCScenario,
                                  ...
                                  post_mcmc_update: Callable,
                                  max_iter: int,
                                  concat_distance: Callable,
threshold_quantile_retain: float,
                                  continuation_func: Callable) -> CDict:
    initial_state.threshold_schedule = np.quantile(concat_distance(initial_state.distance), threshold_quantile_retain)
    ...
    def adaptive_abc_smc_kernel(previous_state: CDict,
        ...
        new_threshold = np.quantile(concat_distance(previous_state.distance), threshold_quantile_retain)
...
def init_abc_smc_sampler_advance(abc_scenario: ABCScenario,
                                 ...
                                 mcmc_steps: int,
                                 post_mcmc_update: Callable,
                                 concat_distance: Callable) -> Callable:
    # Move
    ...
    def smc_sampler_advance(previous_full_state: CDict,
        ...
        sample_inds = random.choice(int_random_key, a=n,
                                    p=concat_distance(previous_full_state.distance) < new_threshold, shape=(n,))
