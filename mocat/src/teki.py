...
#              Iteratively simulate data to get joint empirical mean and cov before conditioning on given data.
#
...
def run_tempered_ensemble_kalman_inversion(scenario: Scenario,
    ...
    if data is None:
        if hasattr(scenario, 'data'):
            data = np.atleast_1d(scenario.data)
        else:
...
def _run_presheduled_teki(scenario: Scenario,
    ...
    simulate_data = vmap(lambda samp, key: scenario.simulate_data(samp, key))
